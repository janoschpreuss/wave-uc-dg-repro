# This is a translation of the MinRes/GMRes solver from NGSolve to work with PETSc vectors
# Adapted from: https://github.com/NGSolve/ngsolve/blob/master/python/krylovspace.py
...
class LinearSolver():
    ...
    def Solve(self, rhs: PETSc.Vec, sol: Optional[PETSc.Vec] = None,
        ...
        if self.estimate_cond:
            return sol,self.residuals, self.condest, self.iterations
        else: 
...
class GMResSolver(LinearSolver):
    ...
    def _SolveImpl(self, rhs: PETSc.Vec, sol: PETSc.Vec):
        ...
        if self.estimate_cond:
            H_mat = np.stack(H).T
            condest = np.linalg.cond(H_mat)
            print("condest = ", condest)
...
def GMRes(A,b,pre=None,x=None,maxsteps = 100, tol = None, innerproduct = None, 
          callback = None, restart = None, startiteration = 0, printrates = True, estimate_cond=False, reltol = None):
